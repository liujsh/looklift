...
> 维护于 2026-08-19（v2.6 垂直修图 Agent Runtime 路线确立）。
...
12. **前端验证分两档,不每次全量** —— 全量验证只在**收口/提交前**跑一次:
   - **迭代中**:`pnpm exec vitest run <受影响的测试文件>` + `pnpm exec tsc --noEmit`。
     不跑全量 vitest,不跑 `pnpm build`。
   - **收口前(一次)**:`pnpm test && pnpm build`。
   - **禁止 `tsc --noEmit` 与 `pnpm build` 同时跑**:本仓 `build` 脚本已是 `tsc && vite build`,
     两者并列 = tsc 跑两遍,纯浪费。
...
- **构建顺序**:v2.0-A/B 已实现 → 完成 M1-M8 并收口 2.0.0 → v2.1 AI Studio → v2.2 平台外壳 →
  v2.3-A 本地图库 → RAW 门 → v2.3-B 设备导入 → v2.4 模板教学 → v2.5 自动化 →
  v2.6 垂直修图 Agent Runtime → v2.7 受控插件。
...
- 示例：
  ...
  - `chore: 更新依赖`
### 分支命名
- 分支名以类型前缀开头，使用 `/` 层级（例如：`feat/...`, `fix/...`, `refactor/...`）
