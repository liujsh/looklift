...
export function LibraryPage({ client, onOpen }: { client: LookliftClient; onOpen(path: string): Promise<void> | void }) {
  ...
  const chooseRoot = async () => { if (!isTauri()) return; const selected = await open({ directory: true, multiple: false, title: "选择图库文件夹" }); if (selected) setPath(selected); };
  const removeRoot = async (id: string) => { await client.removeLibraryRoot(id); setStatus("已移除图库索引，原文件未改变"); await refresh(); };
  ...
  return <main className="coming-soon-page library-page" aria-label="我的图库"><p className="pane-kicker">LIBRARY</p><h1>我的图库</h1><p>只索引原文件，不复制或移动照片。</p><form onSubmit={add}><input value={path} onChange={(event) => setPath(event.target.value)} placeholder="输入本地文件夹路径" required /><button type="button" onClick={() => void chooseRoot()}>选择文件夹</button><button type="submit">加入图库</button></form><form onSubmit={search}><input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索文件名或路径" /><input value={tag} onChange={(event) => setTag(event.target.value)} placeholder="标签" /><button type="submit">搜索</button></form>{roots.map((root) => <p key={root.id}>{root.path} <button type="button" onClick={() => void removeRoot(root.id)}>移除索引</button></p>)}<div className="library-grid">{items.map((item) => <article key={item.id} data-available={item.available}>{item.thumbnail_path && isTauri() ? <img src={convertFileSrc(item.thumbnail_path)} alt="" /> : <span>无缩略图</span>}<strong>{item.display_name}</strong><span>{item.available ? "可用" : "原文件已缺失"}</span><button type="button" disabled={!item.available} onClick={() => void onOpen(item.path)}>进入 Studio</button><button type="button" disabled={!item.available} onClick={() => void reveal(item.path)}>在资源管理器中显示</button><button type="button" onClick={() => void saveTags(item)}>编辑标签</button></article>)}</div>{status && <span role="status">{status}</span>}</main>;
}
