...
export function PlatformShell({ client, contract, engineLabel, store: providedStore, onQuickEdit }: PlatformShellProps) {
  ...
  const recentStudio = studioTabs[studioTabs.length - 1];
  const currentTemplatePhoto = recentStudio?.kind === "studio"
    ? { path: (recentStudio.runtime as StudioRuntime).imagePath, title: (recentStudio.runtime as StudioRuntime).title }
    : null;
  ...
  return (
    <main className="platform-shell" data-navigation-collapsed={navigationCollapsed}>
      ...
      <section className="platform-content" inert={closeDialog ? true : undefined}>
        ...
        {activeTab.kind === "platform" && (activeTab.page === "settings" ? <SettingsPage client={client} /> : activeTab.page === "templates" ? <TemplatePage client={client} contract={contract} canApply={studioTabs.length > 0} currentPhoto={currentTemplatePhoto} onApply={applyTemplate} /> : activeTab.page === "import" ? <ImportPage client={client} /> : activeTab.page === "library" ? <LibraryPage client={client} onOpen={async (path) => {
