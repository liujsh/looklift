...
> 状态:草拟,待作者 review。同迭代:[设计](./design.md) · [任务](./tasks.md)。
...
- [ ] **2048px 代理 <50ms** benchmark(全 operator 全开,非 spike 子集;单机、warmup 后计时);
      benchmark 作为回归测试的软门槛(超阈值告警)
- [ ] 全分辨率 40MP 导出在可接受区间(目标 <1s,记录实测;不达标不阻塞但记入风险)
