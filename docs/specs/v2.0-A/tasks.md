...
按 [design.md §二](./design.md):`exposure`/`white_balance`/`color_grading` 叠色 blend 移到
linear 段(S1 编码进、编码 display 后再走其余 op);color_grading 区权重仍按 display luma。
...
**验收**:2048px 代理单帧 **<50ms**(全 op 全开,单机、warmup 后)作为软门槛回归项(超阈告警);
...
dehaze 所需辅助场(见 T10,简单近似优先)、定种子噪声场(grain 用);融合内核读这些缓冲
...
不确定,超本期)。子步:
- T10a **mini-spike**(先花小预算):试简单局部对比/饱和近似,观感够不够「去雾」方向?够 → 直接实现;
- T10b:若简单近似明显不足,再评估暗通道轻量版 `J=(I−A)/max(t,t0)+A`;仍不达标则**守住简单近似**,
  记入风险、不阻塞本期(dehaze 非核心画质门槛)。
...
**验收**:grain_amount>0 使局部方差增大且中间调最明显、同种子可复现;vignette 迁移后 `test_render.py`
暗角断言全绿;0 值恒等。
