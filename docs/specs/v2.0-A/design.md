...
| `color_grading` | `color_grading.{shadows,midtones,highlights,global_,blending,balance}` | 混合:权重按 display luma,叠色 blend 在 **linear** | 有 |
...
| 光域 | operator | 理由 |
|---|---|---|
| **linear(scene-referred)** | exposure、white_balance、color_grading 的叠色 blend | 光的加/乘性合并,物理上就在线性辐射域;「1 档=2×」只有在 linear 才成立 |
| **display-referred(sRGB OETF 编码后)** | contrast(绕 0.5 中灰)、highlights_shadows、whites_blacks、tone_curve、hsl、saturation、texture、clarity、dehaze、vignette | 这些是相对 display 码值定义的**创作滑杆**:LR 的曲线控制点是 0-255 码值、对比度绕中灰、HSL 在 gamma 编码 RGB 上取 HSV——放进 linear 反而不像 LR |
...
[Stage 1 已是 linear]
  ...
  → color_grading 叠色 blend (linear 混合)        ← 物理合并全在此段
  → 编码 linear→display (sRGB OETF)               ← 唯一一次切换
  → contrast → highlights_shadows → whites_blacks
  → tone_curve → hsl → saturation
  → texture → clarity → dehaze → vignette          ← LR 滑杆语义全在 display 段
  → [Stage 4 输出:display 即成品;本期嵌 sRGB ICC(P3 输出留 v2.x)]
```

> 注:color_grading 的**区权重**(阴影/中间调/高光按亮度)在 display luma 上算,**叠色混合**在
> linear——权重是「哪些像素受影响」的感知划分,混合是「光怎么叠」的物理运算,二者光域不同。
...
- texture/clarity:预算一/两个半径的**高斯模糊亮度**(可分离卷积,自成一个小 numba kernel);
  内核里用 unsharp 残差合成。
- dehaze:预估**全局大气光 A + 粗透射图 t**(暗通道先验的轻量版);内核里 `J=(I-A)/max(t,t0)+A`。
- grain:预生成**噪声场**(定种子,可复现测试);内核里按亮度加权叠加。
...
| **dehaze** ⚠️ **最高研究风险** | **v2.0-A 只做简单近似**:基于**局部对比 + 饱和度**的去霾——在低对比低饱和区提对比、抬饱和、微压黑场(等效「去雾」方向)。**不做完整暗通道先验(dark-channel-prior)**;完整 DCP 的大气光/透射估计精度与性能都不确定,超本期范围 | 若简单近似观感不足,**降级为 mini-spike 任务**(T10 先花小预算验证暗通道轻量版是否可行、可行再实现,否则守住简单近似);负值 → 加雾(抬黑、降对比降饱和) |
...
2. 把 exposure/white_balance/color_grading 叠色**搬进 linear**(S1/S4 编解码)→ 新增线性光
