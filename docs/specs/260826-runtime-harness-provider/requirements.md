...
7. 当页面在桌面宽屏、窄屏、浅色和深色主题下渲染时，布局层级、间距、分段切换、Provider 胶囊选择、表单卡片、字体层级和状态提示应该以已确认的参考界面为基准进行像素级对齐，同时使用 LookLift 自有设计 Token、图标和资源实现。
...
1. 当进行视觉验收时，系统应该以 `references/provider-settings-wide.png`（1280×832）和 `references/provider-settings-narrow.png`（390×844）为基准，在 DPR 1、浅色/深色主题下完成截图对比；参考资源由需求方提供并纳入目录后生效。
