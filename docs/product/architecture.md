...
- `looklift/gui/template_catalog.py` 把既有官方/用户 look 投影为模板卡片；参数仍只存在于
  原 `analysis`，包内 `template_lessons.json` 只保存适用场景、原理和关键参数路径。
- `GET /api/templates` 提供卡片目录，完整参数继续由 `GET /api/looks/<name>` 读取；用户
  look 文件无需迁移，缺少官方教学元数据时从摘要、步骤和非中性参数生成降级教学。
- 平台模板页通过当前最近的 Studio runtime 正式提交模板参数并持久化会话版本；没有 Studio
  时只引导用户先打开照片。聊天模板附件在发送边界组合名称、摘要和白盒参数，附件本身不修改
  正式版本，也不改变安全代理图、provider 或参数白名单边界。
