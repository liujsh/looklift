...
- 迭代中只运行受影响测试与 `pnpm exec tsc --noEmit`；全量回归只在 Task 9 收口时运行一次，且生产构建不与单独的 TypeScript 检查并列。
...
Run: `pnpm exec vitest run src/api/client.test.ts`
...
Run: `pnpm exec vitest run src/store/editorStore.test.ts`
...
Run: `pnpm exec vitest run src/features/chat/chatWorkflow.test.ts src/features/sessions/sessionCoordinator.test.ts src/store/editorStore.test.ts`
...
Run: `pnpm exec vitest run src/components/ChatPane.test.tsx src/app/EditorShell.test.tsx`

Run: `pnpm exec tsc --noEmit`

Expected: 相关测试与 TypeScript 检查成功；本任务未改依赖、Vite 配置或静态资源，不运行 production build。
