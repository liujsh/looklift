...
> 状态:已完成（2026-07-18，作为 2.0.0 内部阶段收口）。同迭代:[设计](./design.md) · [任务](./tasks.md)。
> 上游:[产品需求](../../product/requirements.md)(v2 一站式定位)、
> [当前架构](../../product/architecture.md)(§12 现渲染管线、§15 LUT、§17 强度缩放)、
> [文档规范](../../README.md)。完整调研/决策/spike 实证见本版本 [实施计划](plans/implementation.md)。
...
职责单一、可独测的 operator;只有曝光/白平衡搬进**线性光**做物理正确运算,color grading
整体留在 display 段;整条管线用
...
- **交互级延迟**:2048px 代理 pointwise 融合内核 <50ms;拖滑杆复用 S2 缓存,支撑实时反馈。
...
- [ ] 白平衡在线性光域执行,单测验证其在 linear 而非 gamma 域;color grading 是艺术染色,
      整体留在 display 段;linear 中间值不得在 S4 前提前 clip
...
- [ ] `color_grading.blending`/`balance` 本期仅进入参数契约(contract-only),不产生像素效果
...
- [ ] 对外契约 `render.render(image, analysis) -> Image`、`render.score(rendered, target) -> float`
      签名与语义不变;为保证默认 ICC,CLI/GUI/autorefine 保存点需显式传递渲染结果 profile
...
| **color_grading.blending / balance 像素实现** | 本期 contract-only,像素效果留后续迭代 |
