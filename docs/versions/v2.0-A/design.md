...
> 状态:已完成（2026-07-18，作为 2.0.0 内部阶段收口）。同迭代:[需求](./requirements.md) · [任务](./tasks.md)。
> 基线:[当前架构 §12/§15/§17](../../product/architecture.md)(现渲染管线/LUT/强度缩放)、
> [产品需求](../../product/requirements.md)。参考架构(设计非代码,均不抄码):**AlcedoStudio**
...
[产品需求](../../product/requirements.md) 非目标)。验收判「方向 + 0 值恒等 + 两形态一致」,**不判**与 LR
...
> **S2 预处理有独立性能预算(已裁定)**:pointwise 融合内核的 <50ms(代理)
> 门槛是 spike 只测 pointwise 子集得出的。texture/clarity/dehaze 的**大半径模糊属 S2
> 预处理**,不是 pointwise——代理路径**单独计量、软门槛 <200ms**。全分辨率导出因 S2 大半径 blur 可能**高于
> <1s**,这是**可接受的**(导出非交互);代理路径可在更低分辨率估
