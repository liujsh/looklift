...
> 状态:已完成（2026-07-18，作为 2.0.0 内部阶段收口）。同迭代:[需求](./requirements.md) · [设计](./design.md)。
...
hsl/saturation/color_grading 数学逐字搬进各 operator 的 `apply_numpy`;tone_curve 在 `resolve`
时直接烘成固定 1024 项 1D/C-contiguous/np.float32 LUT,`apply_numpy`/`apply_px` 均消费该 LUT。
(暂全在 display 域),`pipeline.py` 串起来。

**验收**:每个 op 有方向单测(exposure>0 更亮、contrast>0 反差增大、HSV 往返等);现有
`test_render.py` 全绿;tone LUT 与独立 `np.interp` 期望逐项一致,整管线与重构前最大误差
≤1/255(其余 op 仍逐像素一致)。
...
T6b active marshal 测试必须逐项覆盖 enable + 9 个 active 参数字段;其中 tone_curve_lut 与独立
`np.interp` 的 1024 项期望逐项相等,其余标量/数组与 `ResolvedParams` 逐项相等。
