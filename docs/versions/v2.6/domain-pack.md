...
Template 保存场景标签、意图标签、预期效果、禁用条件、风险、兼容 Skill 和受参数契约校验的相对 Patch。首版 Patch
只包含标量 `delta/set`：`delta` 按强度缩放后叠加到正式基线，`set` 在基线当前值与目标值之间按强度插值；它不替换
整套正式参数，也不包含曲线、Prompt 指令、权限、脚本或路径。

首版由用户选择或 UI 使用确定性标签规则推荐，最多一个。Agent 可以不采用、选择 `0..1` 强度、应用后继续修改或
报告不适合。它只能在首个成功候选中应用一次；首候选未采用则本次 Run 记为跳过，后续候选不能补用 Template。
